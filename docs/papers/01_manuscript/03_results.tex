...
However, after the institutional change in 1998 (98-UBR), trends of increasing water use appeared to be effectively suppressed. From 1998 to 2008, the total observed water use decreased by $0.49$ billion $m^3/yr$ per year, while the estimation of water use still suggests $0.82$ billion $m^3/yr$ increases (Figure~\ref{main_results} B).
The increased water uses after 87-WAS aligns with the severe drying-down of the surface streamflow from 1987 to 1998, an obvious indicator of river degradation and environmental crisis (Figure~\ref{main_results}C).
On the other hand, the 98-UBR ended river depletion, despite subsequent increases in drought intensity (from $0.47$ after 87-WAS to $0.62$ after 98-UBR on average) (Figure~\ref{main_results}C).
...
\subsection{Structure-based marginal benefits model}
\label{result-4}
We then compared the marginal returns and optimal water use under three different structural cases theoretically (analogy to Figure~\ref{structure}~C, see \nameref{sec:model}~Figure~\ref{fig:model}, detailed derivation in \textit{Appendix~\nameref{secS4}}).
...
\begin{figure}[!ht]
	...
	\caption{
		The proposed relationship of marginal benefits and water use of individual province under varying cases (case 1 to case 3, corresponding to the different SES structures in Figure~\ref{structure}) Major water users' theoretically optimal water use is also larger (see \nameref{sec:model} and \textit{\nameref{secS4}}).}
	\label{fig:model}
\end{figure}
