...
Then, any weighted average of the control units is a synthetic control and can be represented by a ($J \times 1$) vector of weights $\mathbf{W} = (w_{1}, \ldots ,w_{J})$, with $w_j \in (0, 1)$ and $w_2 + \cdots  + w_{J+1} = 1$.
We denote $k$ is $T \times D$ number of covariates, in which $D$ is number of dimensions of the dataset (i.e., $D = 5$ in this case).
