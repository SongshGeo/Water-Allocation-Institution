...
\title{Approaching social-ecological matches of river basin systems for sustainability}  %! Notice Input
...
\begin{figure*}[!t]
	...
	\caption{
		...
		\textbf{A.} The YBR crosses $10$ provinces or the same-level administrative regions, $8$ of which are highly relying on the water resources from the YRB (see \textit{\ref{secS1}} Table~\ref{tab:quota}). The national administrations are the ultimate authority in issuing water governance policies, which are often implemented by basin-level agency (the Yellow River Conservancy Commission, YRCC) and each province-level agency.
...
% 因此，两次制度转换重塑了，形成了三个由社会行动者和生态节点连接而成的一般性结构
...
\begin{figure}[!htb]
	...
	\caption{
		The proposed relationship of marginal benefits and water use of individual province under varying cases (case 1 to case 3, corresponding to the different SES structures in Figure~\ref{fig:structure}) Major water users' theoretically optimal water use is also larger (see \nameref{sec:model} and \textit{\ref{secS4}}).}
	\label{fig:model}
\end{figure}


For interpretation of the pattern, we compared the theoretical marginal returns and optimal water use under three different structural cases (analogy to Figure~\ref{fig:structure}~C, see \nameref{sec:model}~Figure~\ref{fig:model}, detailed derivation in \textit{\ref{secS4}}).
